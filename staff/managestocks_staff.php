...
<html lang="utf=8">
    ...
    <body>
        ...
        <div id="mainContentSpace" class="main-content" style="">
            ...
            <div id="deleteProductPanel" class="delete-product-panel">
                <!-- debugging span --> <span id="debuggingDel"></span>
                <span style="background-color: transparent; font-size: 30px; font-weight: 400; text-align: center; margin-bottom: 20px">Are you sure you want to <br/> delete this item?</span>
                <div style="display: flex; flex-direction: row; justify-content: space-between; width: 220px; background-color: transparent;">
                    <button id="deleteProductBtn" class="del-product-yes-btn">Yes</button>
                    <button class="del-product-no-btn" onclick="closeDeletePanel()">No</button>
                </div>
            </div>
        ...
        <script>
            // panel visibiltiy
            const deleteProductPanel = document.getElementById('deleteProductPanel');
            const cancelDelProductBtn = document.getElementById('cancelDelProductBtn');
            ...
            // are you sure you want to delete pop-up
            function toggleConfirm() {
                if (deleteProductPanel.style.visibility = 'hidden') {
                    deleteProductPanel.style.visibility = 'visible';
                } else {
            ...
            function closeDeletePanel() {
                if (deleteProductPanel.style.visibility = 'visible') {
                    deleteProductPanel.style.visibility = 'hidden';
                } else {
                    deleteProductPanel.style.visibility = 'visible';
                }
            ...
            document.getElementById('deleteProductBtn').addEventListener('click', (event) => {
                ...
                const formData = {
                    productID: itemToDelete
                };
                console.log(formData);

                // sending form data to backend file
                fetch('http:/StockFlow IMS (FINAL YEAR PROJECT)/db/deleteproduct_db.php', {
                    method: 'POST',
                    headers: {
